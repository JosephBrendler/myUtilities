...
if is_keydev_safe && is_vg_safe && is_luks_safe; then
  ...
  # note: do not need to quote variable expansions inside the array, unless they contain whitespace
  #   but if you do quote, treat the whole argument as one word to be quoted
  qemu_cmd=(
    qemu-system-x86_64
    ...
    -drive "file=${keydev_disk},format=raw,if=ide,cache=none"
...
# currently my kernel lacks the CONFIG_SCSI_VIRTIO support needed for this:
#    -device "virtio-scsi-pci,id=scsi0"
#    -drive "file=${keydev_disk},if=none,id=key"
#    -device "scsi-hd,drive=key,bus=scsi0.0"
#    -drive "file=${luks_disk},if=none,id=luks"
#    -device "scsi-hd,drive=luks,bus=scsi0.0"
