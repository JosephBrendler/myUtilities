...
if echo "$REASON" | grep -E 'IPV6L_RENEW|IPV6_RENEW|IPV6L_UP|IPV6_UP|BOUND|CARRIEER|ROUTERADVERT' >/dev/null; then
    ...
    if [ -z "$ACTION_IPV6_ADDRESS" ]; then
        ...
        if [ -z "$INTERFACE" ] ; then
            ...
            # since no address is passed by dhcpcd, try to use the address generated by the endpoint, if it exists
            # (note: when designed, this would register temp. privacy addresses when no stable ipv6 address
            #  was available, and the calling hook script was intentionally openned to fire on numerous events
            #  including ROUTERADVERT every 20-30 sec, so that ddns updates would be reliably generated for
            #  endpoints coming on line with no other working mechanism to register their address.  However,
            #  this should ignore the temp address [scope global temporary dynamic] and find stable address (RFC7217)
            #  only [scope global dynamic mngtmpaddr noprefixroute] )
#            ACTION_IPV6_ADDRESS=$(ip -6 addr show dev "$INTERFACE" | \
#                grep -E 'inet6[[:space:]]+fd62' | \
#                awk '{print $2}' | \
#                cut -d/ -f1 | \
#                head -n 1)
            # this should work now because of updated /etc/sysctl.d/99joetoo-client-local.conf settings
            # (stable SLAAC addresses are generated by the kernel using RFC 7217: (prefix from RA, interface
            #  identifier derived from a secret, stable per interface + prefix, and are marked "mngtmpaddr"))
            # (temporary privacy SLAAC address are generated by the kernel using RFC 4941: (prefix from RA,
            #  randomized IID, rotate periodically, marked "temporary"))
            # I want stable for registration with dns (inbound traffic) and temporary for outbound traffic
            ACTION_IPV6_ADDRESS=$(ip -6 addr show dev "$INTERFACE" \
                | grep -E 'inet6[[:space:]]+fd62' | \
                | grep -v 'temporary' \
                | awk '{print $2}' \
